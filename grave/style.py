_VALID_NODE_STYLE = ["size", "color", "shape", "width", "edgecolor"]
...
def default_node_style():
    return {"color": "C0", "size": 50, "shape": "o", "width": 1, "edgecolor": "white"}


def default_edge_style():
    return {"color": "#292929", "width": 1, "style": "-"}
...
def default_style():
    return {"node_style": default_node_style, "edge_style": default_edge_style}
...
def use_attributes(keys=None):
    ...
    def inner(attributes):
        if keys is None:
            return {k: attributes[k] for k in _ALL_STYLE_KEYS if k in attributes}
...
def apply_style(style, item_iterable, default):
    ...
    if callable(style):
        ...
        raise TypeError(
            "style must be dict or callable," " got {0}".format(type(style))
        )
    return styles
...
def generate_node_styles(network, node_style):
    ...
    return apply_style(node_style, network.nodes.items(), default_node_style)


def generate_edge_styles(network, edge_style):
    # dict of edge tuple -> edge_style_dict
    return apply_style(edge_style, network.edges.items(), default_edge_style)


def generate_node_label_styles(network, label_style):
    # returns dict of node id -> node_style_dict
    return apply_style(label_style, network.nodes.items(), default_node_label_style)


def generate_edge_label_styles(network, label_style):
    # returns dict of edge tuple -> edge_style_dict
    return apply_style(label_style, network.edges.items(), default_edge_label_style)
