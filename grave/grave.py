...
def _generate_node_artist(pos, styles, *, ax):
    ...
    x = np.zeros(N) * np.nan
    y = np.zeros(N) * np.nan
    properties = {k: [None] * N for k in styles[proto_node] if k in _VALID_NODE_STYLE}
    ...
    node_art = PathCollection(
        paths, offsets=offsets, transOffset=ax.transData, **renamed_properties
    )
...
def _generate_straight_edges(edges, pos, styles, *, ax):
    ...
    edge_pos = [None] * N
    edge_indx = [None] * N
    properties = {k: [None] * N for k in styles[proto_edge] if k in _VALID_EDGE_STYLE}
    ...
    renamed_properties = {key_map[k]: v for k, v in properties.items()}
    line_art = LineCollection(
        edge_pos, transOffset=ax.transData, zorder=1, **renamed_properties
    )
...
def _generate_node_labels(pos, styles, *, ax):
    ...
    for node, nstyle in styles.items():
        ...
        node_labels_dict[node] = Text(
            x, y, label, transform=ax.transData, clip_on=True, **properties
        )
...
class NXArtist(Artist):
    ...
    def _reprocess(self, *, reset_pos=True):
        ...
        self._edge_artist, self._edge_indx = _generate_straight_edges(
            graph.edges(), pos, edge_style_dict, ax=self.axes
        )

        # handle the nodes
        node_style_dict = generate_node_styles(graph, node_style)
        self._node_artist, self._node_indx = _generate_node_artist(
            pos, node_style_dict, ax=self.axes
        )
        ...
        if node_label_style is not None:
            nlabel_style_dict = generate_node_label_styles(graph, node_label_style)
            self._node_label_dict = _generate_node_labels(
                pos, nlabel_style_dict, ax=self.axes
            )

        # handle the edge labels
        if edge_label_style is not None:
            elabel_style_dict = generate_edge_label_styles(graph, edge_label_style)
            self._edge_label_dict = _generate_edge_labels(
                pos, elabel_style_dict, ax=self.axes
            )
    ...
    def contains(self, mouseevent):
        ...
        props["nodes"] = [self._node_indx[j] for j in node_props.get("ind", [])]
...
def plot_network(
    ...
    *,
    ...
    art = NXArtist(
        graph, layout, node_style, edge_style, node_label_style, edge_label_style
    )
    ax.add_artist(art)
    art._reprocess()
